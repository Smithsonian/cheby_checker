...
DATA_PATH = os.getenv('DEV_DATA_DIR')
...
class Observatory:
    ...
    def __init__(self):
        ...
        # Convert obscode.dat lines to geocentric x,y,z positions and
    ...
    def getObservatoryPosition(self, obsCode, jd_utc, xyz=None, velocity=False, old=False):
        ...
        *** If old=True, the old Novas implementation is used. ***
        ...
        if obsCode == '500':
            ...
            geocentric_vec = (Constants.Rearth_AU
                              * np.array(novas.ter2cel(jd_ut1, 0.0, delta_t,
                                                       xp, yp, obsVec)))
        heliocentric_vec = pos + geocentric_vec
        self.observatoryPositionCache[(obsCode, jd_utc)] = heliocentric_vec
        if velocity:
            return heliocentric_vec, vel
    ...
    def getObservatoryPosition_old(self, obsCode, jd_utc, xyz=None):
    ...
    def getEarthP(self, jd_utc, xyz=None):
        jd_tdb  = EOP.jdTDB(jd_utc)
...
class EarthAndTime:
    ...
    def __init__(self, filename1=os.path.join(DATA_PATH, 'finals2000A.all'),
        ...
        nitems = len(jds)
        self.ut1_utc_func = interp1d(jds, ut1_utcs, fill_value=(ut1_utcs[0], ut1_utcs[nitems-1]), bounds_error=False )
        self.pmx_func     = interp1d(jds, pmxs, fill_value=(pmxs[0], pmxs[nitems-1] ), bounds_error=False )
        self.pmy_func     = interp1d(jds, pmys, fill_value=(pmys[0], pmys[nitems-1] ), bounds_error=False )
...
def getEarthPV(jd_tdb, old=False):
    ...
    (bary_Sun_pos, bary_Sun_vel
     ) = jpl_kernel[0, 10].compute_and_differentiate(jd_tdb)
    (bary_EMB_pos, bary_EMB_vel
     ) = jpl_kernel[0, 3].compute_and_differentiate(jd_tdb)
    (EMB_500_pos, EMB_500_vel
     ) = jpl_kernel[3, 399].compute_and_differentiate(jd_tdb)
    helio_500_pos = (-bary_Sun_pos + bary_EMB_pos + EMB_500_pos
                     ) / Constants.au_km  # Convert km to AU
    helio_500_vel = (-bary_Sun_vel + bary_EMB_vel + EMB_500_vel
