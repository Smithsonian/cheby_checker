...
def test_db_creation():
    ...
    # TODO: fetch_db_filepath() doesn't seem to work this way anymore.
    # assert 'sifter' in DB.fetch_db_filepath()
    ...
    assert os.path.isfile(os.path.join(DB.fetch_db_filepath()))
...
def test_tracklet_upsert(tracklet_dict_list):
    ...
    DB.upsert_tracklet(conn, tracklet_dict['JD'], tracklet_dict['HP'],
...
def test_tracklets_upsert(tracklet_dict_list):
    ...
    DB.upsert_tracklets(conn, JD, HP, tracklet_name, tracklet_dict_list)
...
def test_tracklet_query(tracklet_dict_list):
    ...
    list_of_tuples = DB.query_tracklets_jdhp(conn, JD[0], HP[0])
...
def test_tracklet_query_mutiple_HP(tracklet_dict_list):
    ...
    # query the data & check that requisite dictionaries are returned
    list_of_tuples = DB.query_tracklets_jd_hplist(conn, JD[0], HP)
    assert isinstance(list_of_tuples, list) and len(list_of_tuples) == 1
...
def test_delete_tracklet(tracklet_dict_list):
    ...
    DB.delete_tracklet(conn, tracklet_dict_list[0]['tracklet_name'])
    list_of_tuples = DB.query_tracklets_jdhp(conn, JD[0], HP[0])
...
def test_delete_tracklets(tracklet_dict_list):
    ...
    list_of_tuples = DB.query_tracklets_jdhp(conn, JD[0], HP[0])
    assert isinstance(list_of_tuples, list) and len(list_of_tuples) == 1
    list_of_tuples = DB.query_tracklets_jdhp(conn, JD[1], HP[1])
    assert isinstance(list_of_tuples, list) and len(list_of_tuples) == 1

    # now delete two tracklets & check that two fewer dictionaries remain
    DB.delete_tracklets(conn, [tracklet_dict_list[0]['tracklet_name'],
