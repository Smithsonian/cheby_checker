...
assert np.all(V.arr[0] == inputdata)
...
assert P.arr.shape == (1,len(data_classes.var_names['Pointings']) )
assert np.all(P.arr[0] == inputdata)


# (ii) Create Pointings containing input data (2d input)
n = 3
inputdata = np.random.random_sample( (n, len(data_classes.var_names['Pointings'])) )
P = data_classes.Pointings(inputdata)
assert hasattr(P, 'arr')
assert P.arr.shape == (n,len(data_classes.var_names['Pointings']) )
assert np.all(P.arr == inputdata)

# (iv) Demonstrate  __getattribute__ ...
# Here we show that the named attributes are equal to slices of the input array
for var,num in data_classes.var_names['Pointings'].items():
